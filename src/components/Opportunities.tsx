...
const modes = ['Online', 'Offline', 'Hybrid', 'Online / Global'];

const providerTypes = [
  'Government', 'University', 'Company', 'Foundation', 'NGO', 'School',
...
export function Opportunities({
  ...
  const categoryCounts = useMemo(() => {
    return activeOpportunities.reduce<Record<string, number>>((acc, opportunity) => {
  ...
  const filtered = useMemo(() => {
    ...
    return activeOpportunities
      ...
      .filter((o) => funding === 'all' || o.funding === funding)
      .filter((o) => {
        if (grade === 'all') return true;
        const selectedGrade = grade.toLowerCase();
        const gradeMatch = o.grades?.some((g) => g.toLowerCase() === selectedGrade) ?? false;
        const eligibilityMatch = o.eligibility?.toLowerCase().includes(selectedGrade) ?? false;
        return gradeMatch || eligibilityMatch;
      })
      .filter((o) => mode === 'all' || o.mode === mode)
      .filter((o) => providerType === 'all' || o.providerType === providerType)
      .filter((o) => {
        if (!indiaOnly) return true;
        return o.IndiaEligible === true || o.country === 'India' || o.country === 'Online / Global';
      })
      .filter((o) => {
        if (deadline === 'all' || o.deadline === 'Rolling' || o.deadline === 'To be announced' || o.deadline === 'TBA') {
          return true;
        }
      ...
      .filter((o) => {
        ...
        return (
          ...
          o.grades?.some((g) => g.toLowerCase().includes(q)) ||
  ...
  const activeCount =
    [country, grade, funding, mode, providerType].filter((v) => v !== 'all').length +
    (deadline !== 'all' ? 1 : 0) +
    (indiaOnly ? 1 : 0);
  ...
  const reset = () => {
    ...
    setVisibleCategories(CATEGORY_PAGE_SIZE);
  };

  const visibleCategoryItems = categories.slice(0, visibleCategories);
  const canShowMore = visibleCategories < categories.length;
      ...
      <div className="mx-auto max-w-8xl px-5 sm:px-8">
        <Reveal className="max-w-3xl">
          ...
          <p className="mt-4 text-base leading-relaxed text-ink-300">
            Explore scholarships, exams, colleges, competitions, fellowships, internships,
            research programs, courses and more — all in one place.
          </p>
        </Reveal>
        ...
        <Reveal className="mt-5">
          <div className="flex flex-wrap gap-2">
            ...
            {canShowMore && (
              <button
                type="button"
                onClick={() => setVisibleCategories((count) => Math.min(count + CATEGORY_PAGE_SIZE, categories.length))}
                className="inline-flex items-center gap-1.5 rounded-full border border-gold-500/30 bg-gold-500/10 px-3.5 py-1.5 text-xs font-medium text-gold-300 transition-all hover:bg-gold-500/15 hover:text-gold-200"
              >
                Show 20 more
                <ChevronDown className="h-3.5 w-3.5" />
              </button>
            )}
            {!canShowMore && categories.length > CATEGORY_PAGE_SIZE && (
              <button
                type="button"
                onClick={() => setVisibleCategories(CATEGORY_PAGE_SIZE)}
                className="inline-flex items-center gap-1.5 rounded-full border border-white/10 px-3.5 py-1.5 text-xs font-medium text-ink-300 hover:border-white/20 hover:text-white"
              >
                Show less
        ...
        <Reveal className="mt-4">
          <div className="flex items-center justify-between gap-3">
            ...
            >
              ...
              Filters
              {activeCount > 0 && (
                <span className="rounded-full bg-gold-500/20 px-1.5 py-0.5 text-[10px] text-gold-300">{activeCount}</span>
              )}
            </button>
            ...
            <div className="flex items-center gap-3">
              ...
              {(activeCount > 0 || query || activeCategory !== 'all') && (
          ...
          {showFilters && (
            <div className="mt-4 grid gap-4 rounded-xl border border-white/[0.06] bg-ink-850/50 p-4 sm:grid-cols-2 lg:grid-cols-4">
              <FilterSelect label="Grade / Level" value={grade} onChange={setGrade} options={['all', ...grades]} />
              ...
              />
              <FilterSelect label="Mode" value={mode} onChange={setMode} options={['all', ...modes]} />
              <FilterSelect label="Provider" value={providerType} onChange={setProviderType} options={['all', ...providerTypes]} />
              <label className="flex cursor-pointer items-end gap-3 rounded-lg border border-white/[0.08] px-3 py-2.5 text-sm text-ink-200 transition-colors hover:border-white/20">
                ...
                />
                <span>
                  <span className="block text-sm text-white">India eligible</span>
                  <span className="text-[11px] text-ink-400">Show opportunities available to students in India</span>
                </span>
              </label>
            </div>
          )}
        ...
        {filtered.length > 0 ? (
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((opp, i) => (
              <Reveal key={opp.id} delay={Math.min(i * 35, 350)}>
          ...
          <div className="mt-12 flex flex-col items-center justify-center rounded-2xl border border-white/[0.06] bg-ink-850/40 py-16 text-center">
            ...
            <p className="mt-2 max-w-sm text-sm text-ink-300">Try adjusting your filters or searching for another opportunity.</p>
            <button onClick={reset} className="mt-6 inline-flex items-center gap-2 rounded-lg border border-white/10 px-5 py-2.5 text-sm font-medium text-ink-200 hover:border-gold-500/30 hover:text-white">
              <X className="h-4 w-4" /> Clear filters
            </button>
            <a href="/ask" className="mt-3 inline-flex items-center gap-2 text-sm text-gold-300 hover:text-gold-200">
              <MessageCircle className="h-4 w-4" /> Ask Khoj to help
            </a>
          </div>
        )}
      </div>
